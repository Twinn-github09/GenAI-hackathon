...
import NearbyLawyers from './NearbyLawyers';
...
const Main = () => {
  ...
  const handleHelpFormSubmit = async (e) => {
    e.preventDefault();
      ...
      if (data.docx_content) {
        const byteCharacters = atob(data.docx_content);
        ...
        link.download = 'legal_template.docx';
  ...
  return (
    <div className="main">     
      ...
      {showHelpForm ? (
        ...
        <NearbyLawyers onBack={() => setShowLawyerFinder(false)} />
      ) : !showResults ? (
          <>
            <div className="greet">       
              <p>
                <span>Welcome to Nyaya-Sethu !</span>
              </p>
              ...
              <div className="welcome-buttons" style={{ display: 'flex', justifyContent: 'center', gap: '10px' }}>
                ...
                <button className="get-help-button" onClick={() => setShowLawyerFinder(true)}>
  Find Lawyers Nearby
</button>
              </div>
            </div>
